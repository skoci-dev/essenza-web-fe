 ...
 */
export function useRecaptcha(containerId = null, { version = 'v3', autoLoad = true } = {}) {
  ...
  const execute = useCallback(
    async (action = 'form_submit') => {
      ...
      if (version === 'v3') {
        ...
        const result = { captcha_token: token, captcha_version: version }

        setRecaptchaData(result)
  ...
  useEffect(() => {
    if (loaded && version === 'v3') {
      ...
      return () => {
        const styleEl = document.getElementById('recaptcha-badge-style')
